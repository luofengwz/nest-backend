...
export class UploadController {
  constructor(private readonly uploadService: UploadService, private readonly imageService: ImageService, private readonly config: ConfigService) {}
  ...
  uploadImage(@UploadedFile() file) {
    const uFile = this.uploadService.uploadFile(file)
    const path = `${this.config.get('app.file.serveRoot')}/${uFile.day}/${uFile.filename}`
    return ResultData.ok({ path, fullPath: this.config.get('app.file.domain') + path })
  }
}
